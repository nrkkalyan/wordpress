...
if ( !class_exists( 'YWPC_Widget' ) ) {
    ...
    class YWPC_Widget extends WP_Widget {
        ...
        public function __construct() {
            parent::__construct(
                'ywpc_widget', __( 'YITH WooCommerce Product Countdown', 'yith-woocommerce-product-countdown' ), array( 'description' => __( 'Display a list of products with sale timer and/or sale bar', 'yith-woocommerce-product-countdown' ), )
            );
        }
        ...
        public function form( $instance ) {
            ...
            /* $options = '';
             ...
             if ( $posts->have_posts() ) {
                 while ( $posts->have_posts() ) {
                     ...
                     if ( is_array( $instance['product_ids'] ) && @in_array( $posts->post->ID, $instance['product_ids'] ) ) {
            ...
            <p>
                <label for="<?php echo $this->get_field_name( 'title' ); ?>">
                    <?php echo __( 'Widget Title', 'yith-woocommerce-product-countdown' ); ?>
                </label>
            ...
            <p>
                <label for="<?php echo $this->get_field_name( 'product_ids' ); ?>">
                    <?php echo __( 'Products to show', 'yith-woocommerce-product-countdown' ); ?>
                </label>

                <input type="hidden" class="ywpc-wc-product-search" data-multiple="true" name="<?php echo $this->get_field_name( 'product_ids' ); ?>" data-placeholder="<?php _e( 'Search for a product&hellip;', 'yith-woocommerce-product-countdown' ); ?>" data-action="woocommerce_json_search_products" data-selected="<?php
