...
if (!class_exists('YITH_WooCommerce_Order_Tracking_Importer')) {
    ...
    class YITH_WooCommerce_Order_Tracking_Importer extends WP_Importer
    {
        ...
        {
            if (!is_file($file)) {
                $this->import_error(__('The file does not exist, please try again.', 'yith-woocommerce-order-tracking'));
            }
            ...
            if (($handle = fopen($file, "r")) !== false) {
                ...
                if (5 === sizeof($header)) {
                    ...
                    while (($row = fgetcsv($handle, 0, $this->delimiter)) !== false) {
                        ...
                        if (!$order) {
                            $log_message .= sprintf(__('Failed to import row %s. "%s" is not a valid order id.', 'yith-woocommerce-order-tracking'), $row_index, $order_id) . '<br />';
                            continue;
                        }

                        //  Check if the carrier code belong to a known carrier...
                        if (!isset($carriers[strtoupper($carrier_code)])) {
                            $log_message .= sprintf(__('Failed to import row %s: %s is not a known carrier code.', 'yith-woocommerce-order-tracking'), $row_index, $carrier_code) . '<br />';
                            continue;
                        }

                        $shipped_status = ('1' == $shipped_status) ? 'on' : '';

                        update_post_meta($order_id, 'ywot_carrier_id', strtoupper($carrier_code));
                        update_post_meta($order_id, 'ywot_tracking_code', $tracking_code);
                        update_post_meta($order_id, 'ywot_pick_up_date', $shipping_date);
                        update_post_meta($order_id, 'ywot_picked_up', $shipped_status);
                        ...
                        if ($update_status) {
                            $order->update_status('completed', __('Completed order tracking details imported from CSV file', 'yith-woocommerce-order-tracking'));
                        }
                    }

                } else {
                    $this->import_error(__('The CSV file is invalid.', 'yith-woocommerce-order-tracking'));
                }
            ...
            // Show Result
            echo '<div class="updated settings-error below-h2"><p>' . sprintf(__('Operation completed - imported <strong>%s/%s</strong> valid tracking codes.', 'yith-woocommerce-order-tracking'), $imported_codes, $row_index) . '</p></div>';
        ...
        public function fetch_remote_file($url)
        {
            ...
            if (isset($headers['content-length']) && $filesize != $headers['content-length']) {
                ...
                return new WP_Error('import_file_error', __('The size of the remote file is not valid', 'wordpress-importer'));
            }
        ...
        {
            echo '<p>' . __('All done!', 'yith-woocommerce-order-tracking') . '</p>';
        ...
        public function header()
        {
            ...
            echo '<h2>' . __('YITH Order Tracking Importer', 'yith-woocommerce-order-tracking') . '</h2>';
        }
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <small><?php printf(__('Maximum size: %s', 'yith-woocommerce-order-tracking'), $size); ?></small>
                            </td>
                        </tr>
                        <tr>
                            <th>
        ...
        {
            echo '<p><strong>' . __('Sorry, an error has occurred. ', 'yith-woocommerce-order-tracking') . ' </strong ><br />';
            if ($message) {
