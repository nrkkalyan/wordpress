    ...
    class YITH_WCPSC_Admin_Premium extends YITH_WCPSC_Admin {
                    ...
                    'settings' => array(
                        ...
                        'fields' => array(
                            'yith_wcpsc_product_charts' => array(
                                ...
                                'desc'     => __( 'Select size charts you want show in this product', 'yith-wccos' ),
                                'type'     => 'chosen',
                                'options'  => $charts_sel,
                                'std'      => array(),
                                'multiple' => true,
        ...
        public function custom_columns( $column, $post_id ) {
            if ( $column == 'yith_wcpsc_product_size_charts' ) {
                ...
                $html_return      = '';
                if ( !empty( $charts ) ) {
                    foreach ( $charts as $c_id ) {
                        $c = get_post( $c_id );
                        if ( $c ) {
                            $charts_array[]     = $c->post_title;
                            $charts_array_ids[] = $c_id;
                        }
                    }
                }
                if ( !empty( $charts_array ) ) {
        ...
        public function quick_edit_render( $column_name, $post_type ) {
            ...
            if ( $enabled && $post_type == 'product' && $column_name == 'yith_wcpsc_product_size_charts' ) {
                    ...
                    case 'yith_wcpsc_product_size_charts':
                        ...
                        <fieldset class="inline-edit-col-center">
                            <div class="inline-edit-col">
                                <span class="title inline-edit-charts-label">Product Size Charts</span>
                                <!--<input name="yit_metaboxes[yith_wcpsc_product_charts][]" value="0" type="hidden">-->
                                <ul class="charts-checklist product_charts-checklist cat-checklist product_cat-checklist">
                                    ...
                                    if ( !empty( $charts ) ) {
                                        foreach ( $charts as $c ) {
                                            echo "<li id='chart-{$c->ID}'><label class='selectit'><input value='{$c->ID}'
                                                                                   name='yith_wcpsc_product_charts[]'
                                                                                   id='in-chart-{$c->ID}'
                                                                                   type='checkbox'>{$c->post_title}</label>";
                                        }
                                    }
                                    ?>
                                </ul>
        ...
        public function admin_enqueue_scripts() {
            ...
            if ( $is_panel ) {
                wp_enqueue_script( 'yith_wcpsc_panel_js', YITH_WCPSC_ASSETS_URL . '/js/panel.js', array( 'jquery' ), '1.0.0', true );
            }
        ...
        public function register_plugin_for_activation() {
            if ( !class_exists( 'YIT_Plugin_Licence' ) ) {
                require_once( YITH_WCPSC_DIR . 'plugin-fw/lib/yit-plugin-licence.php' );
            }
        ...
        public function register_plugin_for_updates() {
            if ( !class_exists( 'YIT_Upgrade' ) ) {
                require_once( YITH_WCPSC_DIR . 'plugin-fw/lib/yit-upgrade.php' );
            }
...
function YITH_WCPSC_Admin_Premium() {
    return YITH_WCPSC_Admin_Premium::get_instance();
}
