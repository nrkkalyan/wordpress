...
function cxccoo_admin_menu()
{
    add_submenu_page('options-general.php', 
        // 'woocommerce',
        __('Create Customer on Order', 'create-customer-order'), __('Create Customer on Order', 'create-customer-order'), 'manage_options', // 'manage_networks',
        'create_customer_settings', 'cxccoo_admin_page');
}
...
<form method="post" id="mainform" action=""
	...
	<div class="cxccoo-wrap-settings woocommerce">
		...
		<h1><?php _e( 'Create Customer on Order', 'create-customer-order' ); ?></h1>
    ...
    ?>
			...
			<input name="save" class="button-primary" type="submit"
				value="<?php _e( 'Save changes', 'create-customer-order' ); ?>" />
...
function cxccoo_get_settings()
{
    ...
    // Get all the roles that have 'edit_shop_order' role - they are the ones that could possibly use Create Customer on Order.
    $available_users = array();
    foreach ($wp_roles->roles as $r_key => $r_value) {
    ...
    $settings = array(
        ...
        array(
            ...
            'name' => __("Features", 'create-customer-order'),
        ...
        array(
            ...
            'placeholder' => implode(PHP_EOL, array(
                'super_admin',
                'administrator',
                'shop_manager | custom_role',
                'customer',
