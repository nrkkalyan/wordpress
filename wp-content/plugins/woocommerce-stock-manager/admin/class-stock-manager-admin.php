...
class Stock_Manager_Admin
{
    ...
    public function add_plugin_admin_menu()
    {
        ...
        add_menu_page(__('WooCommerce Stock Manager', $this->plugin_slug), __('WooCommerce Stock Manager', $this->plugin_slug), $manage, 'stock-manager', array(
            $this,
         ...
         * );
         */
        add_submenu_page('stock-manager', __('Setting', $this->plugin_slug), __('Setting', $this->plugin_slug), $manage, 'stock-manager-setting', array(
            $this,
    ...
    public function generate_csv_file()
    {
        if (isset($_GET['action']) && $_GET['action'] == 'export') {
            ...
            foreach ($products as $item) {
                ...
                $id = $item->ID;
                if (! empty($product_meta['_sku'][0])) {
                    $sku = $product_meta['_sku'][0];
                } else {
                    $sku = '';
                }
                $product_name = $item->post_title;
                if (! empty($product_meta['_manage_stock'][0])) {
                    $manage_stock = $product_meta['_manage_stock'][0];
                } else {
                    $manage_stock = '';
                }
                if (! empty($product_meta['_stock_status'][0])) {
                    $stock_status = $product_meta['_stock_status'][0];
                } else {
                    $stock_status = '';
                }
                if (! empty($product_meta['_backorders'][0])) {
                    $backorders = $product_meta['_backorders'][0];
                } else {
                    $backorders = '';
                }
                ...
                if ($product_type == 'variable') {
                    ...
                    foreach ($variations_array as $vars) {
                        ...
                        if (! empty($var_meta['_manage_stock'][0])) {
                            $manage_stock = $var_meta['_manage_stock'][0];
                        } else {
                            $manage_stock = '';
                        }
                        if (! empty($var_meta['_stock_status'][0])) {
                            $stock_status = $var_meta['_stock_status'][0];
                        } else {
                            $stock_status = '';
                        }
                        if (! empty($var_meta['_backorders'][0])) {
                            $backorders = $var_meta['_backorders'][0];
                        } else {
                            $backorders = '';
                        }
                        if (! empty($var_meta['_stock'][0])) {
                            $stock = $var_meta['_stock'][0];
                        } else {
                            $stock = '0';
                        }
