...
class WC_Points_Rewards_Manage_Points_List_Table extends WP_List_Table {
	...
	private function get_items() {
		...
		$query = "FROM {$wpdb->prefix}users as users LEFT JOIN {$wpdb->prefix}wc_points_rewards_user_points as points_table ON users.ID = points_table.user_id WHERE 1=1 {$where} GROUP BY points_table.user_id ORDER BY (points_table.points_balance+0) {$order}";

		return array(
			'count'   => $wpdb->get_var( "SELECT COUNT( DISTINCT points_table.user_id ) as found_user_points {$query}" ),
	...
	public function extra_tablenav( $which ) {
		if ( 'top' == $which ) {
			...
			if ( version_compare( WOOCOMMERCE_VERSION, '2.3', '<' ) ) : ?>
				<select id="dropdown_customers" name="_customer_user" style="width:200px">
					<option value=""><?php _e( 'Show All Customers', 'woocommerce-points-and-rewards' ) ?></option>
					<?php
						if ( $customer_id ) {
				...
				wc_enqueue_js( "
					...
					$('select#dropdown_customers').css('width', '250px').ajaxChosen({
						...
						data: {
							...
							default:  '" . esc_js( __( 'Show All Customers', 'woocommerce-points-and-rewards' ) ) . "'
						}
					}, function (data) {
