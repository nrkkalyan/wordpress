...
class WC_Points_Rewards_Product {
	...
	/*
	 *	Function: check_for_variable_points_in_product()
	 *
	 *	Function to ensure that when variations are the same price
	 *	but have different points applied, that the product page will
	 *	show a live points message when a variation is choosen in the dropdown.
	 *
	 * @since 1.4.1
	 *
	 */
	public function check_for_variable_points_in_product( $variations ) {

		// Remove this filter, otherwise it will call itself in a loop
		remove_filter( 'woocommerce_available_variation', array( $this, 'check_for_variable_points_in_product' ) );

		if ( $variations[ 'price_html' ] == '' ) {

			$product = get_product( $variations[ 'variation_id' ] );
			$parent = get_product( $product->parent->post->ID );

			// Check to see if there is a points difference
			if ( method_exists( $parent, 'get_variation_price' ) ) {

				$product_variations = $parent->get_available_variations();

				if ( $this->get_highest_points_variation( $product_variations, $parent->id ) > $this->get_lowest_points_variation( $product_variations, $parent->id ) ) {
					$variations[ 'price_html' ] = '<span class="price">' . $product->get_price_html() . '</span>';
				}
			}
		}
	...
	public function render_product_message() {
		...
		if ( ! $message || ! $points_earned ) {
			...
			if ( class_exists( 'WC_Dynamic_Pricing' ) ) {
				...
				if ( get_post_meta( $product->id, '_pricing_rules', true ) ) {
	...
	public function add_variation_message_to_product_summary( ) {
		...
		if ( method_exists( $product, 'get_available_variations' ) ) {
			// Remove this filter temporarily, otherwise expensive nested calls are executed.
			// This registered filter callback is not needed at this stage.
			remove_filter( 'woocommerce_available_variation', array( $this, 'check_for_variable_points_in_product' ) );
	...
	/**
	 * Get the variation with the lowest points and return the points value
	 *
	 * @since 1.4.1
	 */
	public function get_lowest_points_variation( $variations, $product_id ) {

		// get transient name
		$transient_name = $this->transient_lowest_point_variation( $product_id );
		...
		if ( $points === false ) {
			...
			foreach ( $variations as $key => $variation ) {
				...
				if ( $lowest[ 'points' ] == 0 ) {
					$lowest = array ( 'key' => $key, 'points' => $points );
				}

				// if this is the lowest points value save it
				if ( $points < $lowest['points'] ) {
					$lowest = array ( 'key' => $key, 'points' => $points );
				}
			}
	...
	public function create_variation_message_to_product_summary( $points ) {
		...
		// write the message
		$message = sprintf(
			__( 'Earn up to <strong>%d</strong> %s.', 'woocommerce-points-and-rewards' ),
			$points,
	...
	private function replace_message_variables( $message, $product ) {
		...
		if ( method_exists( $product, 'get_variation_price' ) ) {
			...
			if( !$wc_min_points_earned = $product->wc_min_points_earned ) {
				...
				update_post_meta( $product->id, '_wc_min_points_earned', $wc_min_points_earned );
			}
		}

		// Check to see if the minimum points earned is different from the max points earned, if so, dont show the message
		if ( method_exists( $product, 'get_variation_price' ) && isset( $wc_min_points_earned ) && $wc_min_points_earned != $product->wc_max_points_earned ) {
			return '';
		}
		...
		if ( method_exists( $product, 'get_variation_price' ) ) {
			...
			if ( $this->get_highest_points_variation( $variations, $product->id ) > $product->wc_max_points_earned ) {
				return '';
			}
		}
	 ...
	 * @return int the points earned
	 */
	private static function get_category_points( $product, $order = null ) {
		global $wpdb;

		if ( ! $category_ids = woocommerce_get_product_terms( $product->id, 'product_cat', 'ids' ) )
			return '';
		...
		$category_points_data = $wpdb->get_results( "SELECT woocommerce_term_id AS category_id, meta_value AS points FROM {$wpdb->woocommerce_termmeta} WHERE meta_key = '_wc_points_earned' AND woocommerce_term_id IN ( $category_ids_string );" );
	...
	private static function get_category_max_discount( $product ) {
		...
		$category_discount_data = $wpdb->get_results( "SELECT woocommerce_term_id AS category_id, meta_value AS max_discount FROM {$wpdb->woocommerce_termmeta} WHERE meta_key = '_wc_points_max_discount' AND woocommerce_term_id IN ( $category_ids_string );" );
		$category_discount_array = array();

		if ( $category_discount_array && count( $category_discount_array ) > 0 ) {
			foreach( $category_discount_array as $category ) {
