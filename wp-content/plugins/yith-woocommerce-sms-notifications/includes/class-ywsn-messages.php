...
if ( ! class_exists( 'YWSN_Messages' ) ) {
	...
	class YWSN_Messages {
		...
		public function __construct( $order, $customer = false, $phone = null, $country = '' ) {
			...
			$this->_order        = $order;
			$this->_recipient    = ( ( ! empty( $order ) && $customer ) ? $this->_order->billing_phone : $phone );
			...
			$this->_country_code = strtoupper( ! empty( $order ) ? $this->_order->billing_country : $country );
		...
		private function prepare_sms_content( $message = '' ) {
			...
			if ( $message == '' ) {
				...
				if ( $this->_customer_sms ) {
					...
					$order_status = apply_filters( 'ywsn_order_status', $this->_order->post_status, $this->_order->id );
			 ...
			 */
			$additional_charsets = $this->get_special_charsets( apply_filters( 'ywsn_additional_charsets', array() ) );
		 ...
		 * @author  Alberto Ruggiero
		 */
		private function get_admin_numbers( $order ) {
			...
			if ( wp_get_post_parent_id( $order->id ) != 0 ) {
				...
				$numbers = explode( ',', trim( get_option( 'ywsn_admin_phone' ) ) );
		...
		private function send( $phone, $message ) {
			...
			try {
				...
				$message        = substr( $message, 0, $sms_limit );
			...
			$log_args = array( 'type' => $this->_sms_type, 'order' => $this->_order->id, 'success' => $success, 'status_message' => $status_message, 'phone' => $phone, 'message' => $message );
		...
		private function get_status_customer_message( $status ) {
			...
			$lang    = get_post_meta( $this->_order->id, 'wpml_language', true );
		...
		private function replace_placeholders( $message ) {
			...
			$placeholders = array(
				...
				'{order_status}'     => ( $is_test ? __( 'Status', 'yith-woocommerce-sms-notifications' ) : wc_get_order_status_name( $this->_order->get_status() ) ),
				'{billing_name}'     => ( $is_test ? __( 'Billing name', 'yith-woocommerce-sms-notifications' ) : sprintf( '%s %s', $this->_order->billing_first_name, $this->_order->billing_last_name ) ),
				'{shipping_name}'    => ( $is_test ? __( 'Shipping name', 'yith-woocommerce-sms-notifications' ) : sprintf( '%s %s', $this->_order->shipping_first_name, $this->_order->shipping_last_name ) ),
				'{shipping_method}'  => ( $is_test ? __( 'Shipping method', 'yith-woocommerce-sms-notifications' ) : $this->_order->get_shipping_method() ),
				'{additional_notes}' => ( $is_test ? __( 'Additional Notes', 'yith-woocommerce-sms-notifications' ) : $this->_order->customer_note ),
				'{order_date}'       => ( $is_test ? __( 'Order Date', 'yith-woocommerce-sms-notifications' ) : $this->_order->order_date ),
			);
