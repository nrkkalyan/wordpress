...
if( !class_exists( 'YITH_Woocompare_Frontend' ) ) {
    ...
    class YITH_Woocompare_Frontend {
        ...
        public function fields( $products = array() ) {
            ...
            $dynamic = get_option( 'yith_woocompare_dynamic_attributes' ) == 'yes';
            $custom  = get_option( 'yith_woocompare_custom_attributes' ) == 'yes';
                ...
                foreach ( $fields as $field => $name )  {
                    ...
                    switch( $field ) {
                        case 'price':
                            ...
                            break;
                        case 'description':
                            $description = ( get_option( 'yith_woocompare_use_full_description' ) == 'yes' ) ? $product->post->post_content : apply_filters( 'woocommerce_short_description', $product->post->post_excerpt );
                        ...
                        default:
                            ...
                            elseif( $custom_attributes != false ) {

                                if( ! isset( $custom_attributes[$field] ) || ! is_array( $custom_attributes[$field] ) ) {
                                    continue;
                                }

                                $current_custom_attribute = $custom_attributes[$field];

                                if( ! empty( $current_custom_attribute['value'] ) ) {
                                    $product->fields[$field] = str_replace('|', ',', $current_custom_attribute['value'] );
                                }
                            }
                            else {
        ...
        protected function _vars( $products = array() ) {
            ...
            $show_title     = get_option( 'yith_woocompare_fields_product_info_title' ) == 'yes';
	        $show_image     = get_option( 'yith_woocompare_fields_product_info_image' ) == 'yes';
	        $show_add_cart  = get_option( 'yith_woocompare_fields_product_info_add_cart' ) == 'yes';
	    ...
	    public function add_product_to_compare_ajax() {
		    ...
		    $json = array(
			    ...
			    'only_one'      => ( get_option( 'yith_woocompare_open_after_second' ) == 'yes' && count( $this->products_list ) <= 1 ) ? true : false
		    );
