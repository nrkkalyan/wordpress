...
if ( !class_exists( 'YITH_WCBSL_Frontend_Premium' ) ) {
    ...
    class YITH_WCBSL_Frontend_Premium extends YITH_WCBSL_Frontend {
        ...
        public function __construct() {
            $this->reports = new YITH_WCBSL_Reports_Premium();

            // add frontend css
            add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_scripts' ) );

            // Add Shortcode for Best Sellers
        ...
        public function yit_theme_products_in_slider( $query, $args ) {
            ...
            $range_args          = array();
            $per_page            = !empty( $args[ 'per_page' ] ) ? $args[ 'per_page' ] : YITH_WCBSL_Premium()->get_limit();
        ...
        /**
         * Add Best Seller Icon in position setted in Settings Tab
         *
         * @access   public
         * @since    1.0.0
         * @author   Leanza Francesco <leanzafrancesco@gmail.com>
        ...
        public function print_icon() {
            ...
            $is_bestseller = $this->reports->check_is_bestseller( $product->id, $range );
            if ( !$is_bestseller )
                return;
        ...
        public function print_position_in_bestsellers() {
            ...
            $args = array(
                'id'            => $product->id,
                'bestseller_in' => $this->reports->check_is_bestseller( $product->id, $range )
            );

            wc_get_template( '/single-product/position-in-bestsellers.php', $args, YITH_WCBSL_TEMPLATE_PATH, YITH_WCBSL_TEMPLATE_PATH );
        }
        ...
        public function shortcode_handler( $atts, $content = null ) {
            ob_start();
            wc_get_template( 'bestsellers.php', $atts, YITH_WCBSL_TEMPLATE_PATH . '/', YITH_WCBSL_TEMPLATE_PATH . '/' );
        ...
        public function slider_shortcode_handler( $atts, $content = null ) {
            ob_start();
            wc_get_template( 'slider/bestsellers.php', $atts, YITH_WCBSL_TEMPLATE_PATH . '/', YITH_WCBSL_TEMPLATE_PATH . '/' );
        ...
        public function add_badge_in_shop() {
            ...
            if ( !$this->display_badge_on_bestseller( $product->id ) )
                return;
        ...
        public function display_badge_on_bestseller( $product_id ) {
            ...
            if ( $badge_only_for_top ) {
                return is_array( $bestseller_info ) && isset( current( $bestseller_info )[ 'title' ] ) && 'yith_wcbsl_all' === current( $bestseller_info )[ 'title' ];
            } else {
        ...
        public function enqueue_scripts() {
            wp_enqueue_style( 'yith_wcbsl_frontend_style', YITH_WCBSL_ASSETS_URL . '/css/frontend_premium.css' );
            wp_add_inline_style( 'yith_wcbsl_frontend_style', $this->get_inline_css() );

            wp_enqueue_style( 'dashicons' );

            wp_enqueue_script( 'yith_fl_slider_js', YITH_WCBSL_ASSETS_URL . '/js/yith_fl_slider.js', array( 'jquery' ), '1.0.0', true );
            wp_enqueue_script( 'yith_wcbsl_frontend_js', YITH_WCBSL_ASSETS_URL . '/js/frontend.js', array( 'jquery', 'yith_fl_slider_js' ), '1.0.0', true );
        }
...
function YITH_WCBSL_Frontend_Premium() {
    return YITH_WCBSL_Frontend_Premium::get_instance();
}
