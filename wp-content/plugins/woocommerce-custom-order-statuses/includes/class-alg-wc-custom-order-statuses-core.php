...
class Alg_WC_Custom_Order_Statuses_Core {
	...
	function register_order_custom_status_bulk_actions( $bulk_actions ) {
		...
		if ( ! empty( $custom_order_statuses ) && is_array( $custom_order_statuses ) ) {
			foreach ( $custom_order_statuses as $slug => $label ) {
				$bulk_actions[ 'mark_' . substr( $slug, 3 ) ] = sprintf(__( 'Change status: ', 'woocommerce' ) . ' %s',  $label);
			}
		}
		return $bulk_actions;
	}
	...
	function bulk_admin_footer() {
		...
		if ( 'shop_order' == $post_type ) {
			...
			foreach( alg_get_order_statuses() as $key => $order_status ) {
				...
				?>jQuery(function() {
					jQuery('<option>').val('mark_<?php echo $key; ?>').text('<?php echo sprintf(__( 'Change status: ', 'woocommerce' ) . ' %s',  $order_status); ?>').appendTo('select[name="action"]');
					jQuery('<option>').val('mark_<?php echo $key; ?>').text('<?php echo sprintf(__( 'Change status: ', 'woocommerce' ) . ' %s',  $order_status); ?>').appendTo('select[name="action2"]');
				});<?php
			}
			?></script><?php
		}
