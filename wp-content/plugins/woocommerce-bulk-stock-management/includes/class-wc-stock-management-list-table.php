...
class WC_Stock_Management_List_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		if (empty($_REQUEST['filter_product_type'])) {
		    $_REQUEST['filter_product_type'] = 'product';
        }
	}
		...
		switch ( $column_name ) {
			...
			case 'stock' :
				...
				if ($product->is_type( 'variable' )) {
				    ?>
                    <a href="<?php echo admin_url( 'edit.php?post_type=product&page=woocommerce-bulk-stock-management&filter_product_type=product_variation&s=' . $product->get_sku() ) ?>"><?php echo __( 'Variations', 'woocommerce' ) ?></a>
				    <?php
                } else {
                    ?>
                    <input type="text" class="input-text wc_bulk_stock_quantity_value"
                           tabindex="<?php echo $this->index; ?>"
                           data-name="stock_quantity[<?php echo $product->get_id(); ?>]" placeholder="<?php
                    ...
                    <input type="hidden" class="input-text"
                           data-name="current_stock_quantity[<?php echo $product->get_id(); ?>]"
                           value="<?php if (!$product->is_type('variation') || $product->managing_stock()) {
			...
			case 'backorders' :
				if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
					echo '<mark class="yes">' . __( 'Allow, but notify customer', 'woocommerce' ) . '</mark>';
				} elseif ( $product->backorders_allowed() ) {
					echo '<mark class="yes">' . __( 'Allow', 'woocommerce' ) . '</mark>';
				} else {
					echo '<span class="na">' . __( 'Do not allow', 'woocommerce' ) . '</span>';
				}
			break;
		} // End switch().
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'backorders'   => __( 'Backorders?', 'woocommerce' ),
			'stock'        => __( 'Stock qty', 'woocommerce' ),
		);
	...
	public function display_tablenav( $which ) {
		...
		if ( 'top' == $which ) {
			...
			<ul class="subsubsub">
				<li class="product"><a href="<?php echo admin_url( 'edit.php?post_type=product&page=woocommerce-bulk-stock-management&filter_product_type=product' ) ?>" class="<?php if ( ! empty( $_REQUEST['filter_product_type'] ) && 'product' == $_REQUEST['filter_product_type'] ) { echo 'current'; } ?>"><?php _e( 'Products', 'woocommerce' ); ?></a> |</li>
				<li class="variation"><a href="<?php echo admin_url( 'edit.php?post_type=product&page=woocommerce-bulk-stock-management&filter_product_type=product_variation' ) ?>" class="<?php if ( ! empty( $_REQUEST['filter_product_type'] ) && 'product_variation' == $_REQUEST['filter_product_type'] ) { echo 'current'; } ?>"><?php _e( 'Variations', 'woocommerce' ); ?></a></li>
			</ul>

			<?php $this->search_box( __( 'Search', 'woocommerce' ), 'search-products' );
		}
		?>
			...
			<?php if ( 'top' == $which ) : ?>
				...
				<div class="alignleft actions">
					...
					<select name="filter_manage_stock">
						...
						<option value="yes" <?php if ( ! empty( $_REQUEST['filter_manage_stock'] ) && 'yes' == $_REQUEST['filter_manage_stock'] ) { selected( 1 ); } ?>><?php echo __( 'Manage stock', 'woocommerce' ) . ' -> ' . __( 'Allow' ); ?></option>
						<option value="no" <?php if ( ! empty( $_REQUEST['filter_manage_stock'] ) && 'no' == $_REQUEST['filter_manage_stock'] ) { selected( 1 ); } ?>><?php echo __( 'Manage stock', 'woocommerce' ) . ' -> ' . __( 'Do not llow' ); ?></option>
					</select>
					<select name="filter_stock_status">
						<option value=""><?php _e( 'Any stock status', 'woocommerce-bulk-stock-management' ); ?></option>
						<option value="instock" <?php if ( ! empty( $_REQUEST['filter_stock_status'] ) && 'instock' == $_REQUEST['filter_stock_status'] ) { selected( 1 ); } ?>><?php _e( 'In stock', 'woocommerce' ); ?></option>
						<option value="outofstock" <?php if ( ! empty( $_REQUEST['filter_stock_status'] ) && 'outofstock' == $_REQUEST['filter_stock_status'] ) { selected( 1 ); } ?>><?php _e( 'Out of stock', 'woocommerce' ); ?></option>
					</select>
					<?php
						global $wp_query;
					...
					if ( $terms ) {
						...
						echo '<option value="0" ' . selected( isset( $_REQUEST['filter_product_cat'] ) ? $_REQUEST['filter_product_cat'] : '', '0', false ) . '>' . __( 'Uncategorized' ) . '</option>';
						?>
							</select>
							<?php
					}
					...
					<input type="submit" name="filter" value="<?php _e( 'Filter', 'woocommerce' ); ?>" class="button" />
				</div>
			<?php else : ?>
				<div class="alignleft actions">
					<?php $this->bulk_actions( $which ); ?>
