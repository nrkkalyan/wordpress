...
class WC_POS
{
    ...
    {
        if (!isset($_GET['filter']['updated_at_min']) && !is_admin() && (isset($query->query_vars['post_type']) && $query->query_vars['post_type'] == 'product') || (is_product_category() && !isset($query->query_vars['post_type']))) {
            $meta_query = array(
                'relation' => 'OR',
                array(
                    'key' => '_pos_visibility',
