...
#	        add_action( 'wp_login', array( $this, 'set_last_login') );
    ...
    public function inline_js_payment_gateway($inline_js)
    {
        ...
        if (in_array('stripe', $this->active_plugins) && in_array('stripe', $enabled_gateways)) {
            ...
            /*$payment_request = new WC_Stripe_Payment_Request();
            $wc_stripe_payment_request_params = array(
                'ajax_url' => WC_AJAX::get_endpoint('%%endpoint%%'),
                'stripe' => array(
                    'key' => $payment_request->publishable_key,
                ...
                'button' => array(
                    'type' => $payment_request->get_button_type(),
                    'theme' => $payment_request->get_button_theme(),
                    'height' => $payment_request->get_button_height(),
                    'locale' => substr(get_locale(), 0, 2), // Default format is en_US.
                ),
                'is_product_page' => is_product(),
                'product' => $payment_request->get_product_data(),
            );

            $inline_js[] = '<script type="text/javascript" class="wc_stripe_params" >  var wc_stripe_payment_request_params = ' . json_encode($wc_stripe_payment_request_params) . '; </script>';*/
        }
        if (in_array('simplify_commerce', $enabled_gateways)) {
