 ...
 * @version     1.0.0
 */
class WC_POS_Report_Sales_By_Cashier extends WC_Admin_Report {

public $chart_colours      = array();
	public $cashier_ids         = array();
	public $cashier_ids_titles = array();
	private $report_data;
	...
	public function __construct() {
		if ( isset( $_GET['cashier_ids'] ) && is_array( $_GET['cashier_ids'] ) ) {
			$this->cashier_ids = array_filter( array_map( 'absint', $_GET['cashier_ids'] ) );
		} elseif ( isset( $_GET['cashier_ids'] ) ) {
			$this->cashier_ids = array_filter( array( absint( $_GET['cashier_ids'] ) ) );
		}
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title'            => sprintf( __( '%s gross sales', 'wc_point_of_sale' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the order totals after any refunds and including shipping and taxes.', 'woocommerce' ),
			'color'            => $this->chart_colours['sales_amount'],
			'highlight_series' => 6
		);
		$legend[] = array(
			'title'            => sprintf( __( '%s net sales', 'wc_point_of_sale' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
	...
	public function current_filters() {
		...
		$this->cashier_ids_titles = array();

		foreach ( $this->cashier_ids as $cashier_id ) {
			...
			if ( $user ) {
				...
				$this->cashier_ids_titles[] = $display_name;
			} else {
				$this->cashier_ids_titles[] = '#' . $cashier_id;
			}
		}

		echo '<p>' . ' <strong>' . implode( ', ', $this->cashier_ids_titles ) . '</strong></p>';
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'cashier_ids' ) ) . '">' . __( 'Reset', 'woocommerce' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php _e( 'Search Cashier', 'wc_point_of_sale' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<input type="hidden" class="wc-product-search" style="width:203px;" name="cashier_ids[]" data-placeholder="<?php _e( 'Search for a register&hellip;', 'wc_point_of_sale' ); ?>" data-action="wc_pos_json_search_cashier" />
		...
		<h4 class="section_title"><span><?php _e( 'Top cashiers', 'wc_point_of_sale' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				...
				if ( $top_sellers ) {
					foreach ( $top_sellers as $cashier ) {

						$user = get_user_by('id', $cashier->cashier_id);
						...
						if ( $user ) {
							...
							$user_name = '#' . $cashier->cashier_id;
						}

						echo '<tr class="' . ( in_array( $cashier->cashier_id, $this->cashier_ids ) ? 'active' : '' ) . '">
							<td class="count">' . $cashier->count . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'cashier_ids', $cashier->cashier_id ) ) . '">' . $user_name . '</a></td>
							<td class="sparkline">' . $this->event_sales_sparkline( $cashier->cashier_id, 7, 'count' ) . '</td>
						</tr>';
					}
				} else {
		...
		<div class="section">
			<table cellspacing="0">
				...
				if ( $top_earners ) {
					foreach ( $top_earners as $cashier ) {
						$user = get_user_by('id', $cashier->cashier_id);
						...
						echo '<tr class="' . ( in_array( $cashier->cashier_id, $this->cashier_ids ) ? 'active' : '' ) . '">
							<td class="count">' . wc_price( $cashier->order_total ) . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'cashier_ids', $cashier->cashier_id ) ) . '">' . $user_name . '</a></td>
							<td class="sparkline">' . $this->event_sales_sparkline( $cashier->cashier_id, 7, 'sales' ) . '</td>
						</tr>';
					}
				} else {
					echo '<tr><td colspan="3">' . __( 'No items found.' ) . '</td></tr>';
	...
	public function get_main_chart() {
		...
		if ( ! $this->cashier_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a cashier to view stats', 'wc_point_of_sale' ); ?></p>
			</div>
			<?php
		} else {
