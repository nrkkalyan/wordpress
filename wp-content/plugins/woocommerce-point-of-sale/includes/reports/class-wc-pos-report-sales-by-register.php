 ...
 * @version     1.0.0
 */
class WC_POS_Report_Sales_By_Register extends WC_Admin_Report {
	...
	public function __construct() {
		if ( isset( $_GET['register_ids'] ) && is_array( $_GET['register_ids'] ) ) {
			$this->register_ids = array_filter( array_map( 'absint', $_GET['register_ids'] ) );
		} elseif ( isset( $_GET['register_ids'] ) ) {
			$this->register_ids = array_filter( array( absint( $_GET['register_ids'] ) ) );
		}
	}
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title'            => sprintf( __( '%s gross sales from this register', 'wc_point_of_sale' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the order totals after any refunds and including shipping and taxes.', 'woocommerce' ),
			'color'            => $this->chart_colours['sales_amount'],
			'highlight_series' => 6
		);
		$legend[] = array(
			'title'            => sprintf( __( '%s net sales from this register', 'wc_point_of_sale' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
	...
	public function current_filters() {
		...
		foreach ( $this->register_ids as $register_id ) {
	      ...
	      $register_name = $register['name'];
				$this->register_ids_titles[] = $register_name;
			} else {
				$this->register_ids_titles[] = '#' . $register_id;
			}
		}

		echo '<p>' . ' <strong>' . implode( ', ', $this->register_ids_titles ) . '</strong></p>';
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'register_ids' ) ) . '">' . __( 'Reset', 'woocommerce' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php _e( 'Register Search', 'wc_point_of_sale' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<input type="hidden" class="wc-product-search" style="width:203px;" name="register_ids[]" data-placeholder="<?php _e( 'Search for a register&hellip;', 'wc_point_of_sale' ); ?>" data-action="wc_pos_json_search_registers" />
		...
		<h4 class="section_title"><span><?php _e( 'Top Registers', 'wc_point_of_sale' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				$top_sellers = $this->get_order_report_data( array(
					'data' => array(
						...
						'wc_pos_id_register' => array(
							'type'     => 'meta',
							'function' => 'COUNT',
							'name'     => 'count, meta_wc_pos_id_register.meta_value as register_id'
						)
					),
					'where_meta' => array(
						array(
				...
				if ( $top_sellers ) {
					foreach ( $top_sellers as $register ) {
						$reg      = WC_POS()->register()->get_data($register->register_id);
			      $reg      = $reg[0];
			      $reg_name = $reg['name'];
						...
						echo '<tr class="' . ( in_array( $register->register_id, $this->register_ids ) ? 'active' : '' ) . '">
							<td class="count">' . $register->count . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'register_ids', $register->register_id ) ) . '">' . $reg_name . '</a></td>
							<td class="sparkline">' . $this->event_sales_sparkline( $register->register_id, 7, 'count' ) . '</td>
						</tr>';
					}
				} else {
			...
			<table cellspacing="0">
				...
				if ( $top_earners ) {
			      ...
			      $reg_name = $reg['name'];
						...
						echo '<tr class="' . ( in_array( $register->register_id, $this->register_ids ) ? 'active' : '' ) . '">
							<td class="count">' . wc_price( $register->order_total ) . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'register_ids', $register->register_id ) ) . '">' . $title . '</a></td>
							<td class="sparkline">' . $this->event_sales_sparkline( $register->register_id, 7, 'sales' ) . '</td>
						</tr>';
					}
				} else {
					echo '<tr><td colspan="3">' . __( 'No items found.' ) . '</td></tr>';
	...
	public function get_main_chart() {
		...
		if ( ! $this->register_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a register to view stats', 'wc_point_of_sale' ); ?></p>
			</div>
			<?php
		} else {
