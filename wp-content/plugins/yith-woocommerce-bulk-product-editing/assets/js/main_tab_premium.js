jQuery( function ( $ ) {
    var is_debug                             = false,
        ...
        set_html_value                       = function ( a, b ) {
            ...
            if ( b == custom_input_categories || b == custom_attr_input || b == custom_chosen ) {
                var val = b.find( '.chosen' ).val() != null ? b.find( '.chosen' ).val() : '';
        ...
        table_init                           = function () {
            ...
            table.on( 'click', 'td.stock_status, td.manage_stock, td.sold_individually, td.featured, td.virtual, td.downloadable, td.enable_reviews', function () {
            ...
            var extra_class_text = extra_class.text,
                text_classes     = [ 'title', 'regular_price', 'sale_price', 'sku', 'tags', 'weight', 'height', 'width', 'length', 'stock_quantity', 'download_limit',
                                     'download_expiry', 'menu_order', 'button_text', 'product_url', 'slug', 'up_sells', 'cross_sells' ];
            ...
            table
                .on( 'click', 'td', function ( event ) {
                    ...
                    if ( selected.hasClasses( text_classes, 'OR' ) ) {
                        ...
                        custom_input_categories.find( '.chosen' ).trigger( "chosen:updated" );
                        custom_input_categories.show();
                        custom_input_categories.offset( selected.offset() );
                        custom_input_categories.find( '.chosen' ).trigger( 'chosen:open' );
                        ...
                        custom_chosen.find( '.chosen' ).trigger( "chosen:updated" );
                        custom_chosen.show();
                        custom_chosen.offset( selected.offset() );
                        custom_chosen.find( '.chosen' ).trigger( 'chosen:open' );
                        ...
                        current_cell      = $( event.target ).closest( 'td' ); //used for fo_to_next_cell
                        custom_attr_input.width( selected.width() );
        ...
        reset_bulk_editor                    = function () {
            ...
            for ( i in extra_bulk_chosen ) {
                ...
                this_chosen.chosen( 'destroy' );
                this_chosen.prop( 'selectedIndex', -1 );
                this_chosen.chosen( { width: '100%' } );
            }

            b_categories_val.chosen( 'destroy' );
            b_categories_val.prop( 'selectedIndex', -1 );
            b_categories_val.chosen( { width: '100%' } );
        ...
        reset_filters                        = function () {
            ...
            /*f_categories.chosen( 'destroy' );
             f_categories.prop( 'selectedIndex', -1 );
             f_categories.chosen( { width: '95%' } );
             f_tags.chosen( 'destroy' );
             f_tags.prop( 'selectedIndex', -1 );
             f_tags.chosen( { width: '95%' } );
    ...
    filter_form.find( '.chosen' ).chosen( { width: '95%' } );

    yith_wcbep_chosen.chosen( { width: '100%' } );
    b_attributes_val.chosen( { width: '100%' } );
    ...
    custom_input_image.find( 'img' ).on( 'click', function ( e ) {
        e.preventDefault();
        var this_image = $( this );
        var image      = wp.media( {
                                       title   : 'Upload Image',
        ...
        for ( var index in my_checked_rows ) {
                ...
                if ( (tags_v != '' || tags_s == 'del') && tags_cell.find( '.not_editable' ).length < 1 ) {
                    switch ( tags_s ) {
                        ...
                        case 'app':
                            ...
                            break;
                        case 'rep':
                            tags_new = tags_old_value.replace( new RegExp( tags_v, "g" ), tags_r );
                            break;
                        case 'del':
                            tags_new = '';
                            break;
            ...
            for ( var i in textarea_array ) {
                if ( current_matrix_keys.indexOf( textarea_array[ i ] ) > -1 ) {
                    ...
                    if ( v != '' && cell.find( '.not_editable' ).length < 1 ) {
                        switch ( s ) {
                            case 'new':
                                new_value = v;
                                break;
            ...
            // for all checkbox
            var checkbox_array = [ 'manage_stock', 'stock_status', 'sold_individually', 'featured', 'virtual', 'downloadable', 'enable_reviews' ];
            ...
            // for all selects
            var select_array = [ 'tax_status', 'tax_class', 'allow_backorders', 'shipping_class', 'status', 'visibility', 'download_type', 'prod_type' ];
