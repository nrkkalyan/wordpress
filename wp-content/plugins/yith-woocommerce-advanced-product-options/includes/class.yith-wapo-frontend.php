        ...
        public function get_item_data( $other_data, $cart_item ) {
            ...
            if ( ! empty( $cart_item['yith_wapo_options'] ) ) {
                ...
                if( ( is_object( $base_product ) && get_option( 'yith_wapo_settings_show_product_price_cart' ) == 'yes' ) && ( isset( $cart_item['yith_wapo_sold_individually'] ) && ! $cart_item['yith_wapo_sold_individually'] ) ) {
                    ...
                    if( $base_product->product_type == 'yith_bundle' && $base_product->per_items_pricing == true && function_exists('YITH_WCPB_Frontend_Premium') && method_exists( YITH_WCPB_Frontend_Premium() , 'format_product_subtotal' ) ) {
                        $price = YITH_WCPB_Frontend_Premium()->calculate_bundled_items_price_by_cart( $cart_item );
                    } else {
                        $price = $base_product->get_display_price();
                    }
                ...
                foreach ( $type_list as $single_type_options ) {
                    ...
                    if ( $price >= 0 ) {
                        ...
                        if( $price > 0 && ! $this->hide_price( $base_product->id ) ) {
                            $name .= ' (' . wc_price( $price ) . ')';
                        }
                ...
                switch( $price_type ) {
                    ...
                    case 'percentage':
                        ...
                        if( $forced_product_price > 0 ) {
                            ...
                            $product_price = defined('YWCRBP_PREMIUM') ? $product_object->get_price() : $product_object->price;

                            $product_final_price = ( $use_display ? $product_object->get_display_price() : $product_price );
