...
if ( ! class_exists( 'YITH_WCCL_Frontend' ) ) {
	...
	class YITH_WCCL_Frontend {
		...
		public function __construct() {
			...
			// increase wc ajax variation threshold
			add_filter( 'woocommerce_ajax_variation_threshold', array( $this, 'increase_variation_thresold' ), 10, 2 );
		...
		public function init_variables(){
			...
			$this->_create_custom_attributes_array( $product );
		}

		/**
		 * Increase WooCommerce variation thresold
		 *
		 * @param int $thresold
		 * @param object $product
		 * @return int
		 */
		public function increase_variation_thresold( $thresold, $product ) {
			return 150;
		}
		...
		public function enqueue_scripts(){
			...
			wp_localize_script( 'yith_wccl_frontend', 'yith_wccl_general', array(
				...
				'view_cart'     => esc_attr__( 'View Cart', 'yith-woocommerce-product-add-ons' ),
		...
		public function create_attributes_json( $product_id = false, $return = false ) {
			...
			empty( $this->single_product_attributes ) && $this->_create_custom_attributes_array( $product );
			...
			foreach( $attributes as $attribute ) {
				...
				if ( isset( $attribute['is_taxonomy'] ) && $attribute['is_taxonomy'] ) {
					...
					if( array_key_exists( $attribute_taxonomy->attribute_type, $custom_tax ) ) {
						...
						$terms = wc_get_product_terms( $product->id, $taxonomy_name, array( 'fields' => 'all' ) );
						...
						foreach ( $terms as $term ) {
							...
							// add terms values
							$this->single_product_attributes[ $taxonomy_name ]['terms'][ $term->slug ] = array( 'value' => $value, 'tooltip' => $tooltip );
						}
					}
		...
		public function wc_quick_view_json_product_attr(){
			...
			$attr = $this->create_attributes_json( $product->get_id(), true );
