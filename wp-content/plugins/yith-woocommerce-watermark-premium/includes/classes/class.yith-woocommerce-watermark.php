...
if (! class_exists('YITH_WC_Watermark')) {
    ...
    class YITH_WC_Watermark
    {
        ...
        public function __construct()
        {
            ...
            // Add tab premium
            add_action('yith_wc_watermark_premium', array(
                $this,
                'premium_tab'
            ));
            ...
            add_action('wp_ajax_yith_apply_all_watermark', array(
                $this,
                'yith_apply_all_watermark'
            ));
            add_action('wp_ajax_nopriv_yith_apply_all_watermark', array(
                $this,
                'yith_apply_all_watermark'
            ));
            
            // add ajax action for remove watermark
            add_action('wp_ajax_ywcwat_remove_watermark', array(
                $this,
                'ywcwat_remove_watermark'
            ));
            add_action('wp_ajax_nopriv_ywcwat_remove_watermark', array(
                $this,
                'ywcwat_remove_watermark'
            ));
            ...
            if (is_admin()) {
                ...
                add_action('woocommerce_admin_field_custom-button', array(
                    $this,
                    'show_backup_btn'
                ));
                add_action('admin_enqueue_scripts', array(
                    &$this,
                    'admin_enqueue_free_scripts'
                ));
            }
            ...
            add_action('save_post', array(
                $this,
                'generate_watermark_on_save_product'
            ), 10, 1);
            add_filter('wp_generate_attachment_metadata', array(
                $this,
                'generate_watermark_on_attach_image'
            ), 10, 2);
        }
         ...
         * @return mixed @use plugin_action_links_{$plugin_file_name}
         */
        ...
        {
            if ((defined('YWCWAT_INIT') && (YWCWAT_INIT == $plugin_file)) || (defined('YWCWAT_FREE_INIT') && (YWCWAT_FREE_INIT == $plugin_file))) {
        ...
        public function admin_enqueue_free_scripts()
        {
            if (isset($_GET['page']) && 'yith_ywcwat_panel' == $_GET['page']) {
                wp_enqueue_script('ywcwat_free_admin_script', YWCWAT_ASSETS_URL . 'js/ywcwat_admin' . $this->_suffix . '.js', array(
                    'jquery'
                ), YWCWAT_VERSION, true);
                ...
                $ywcwat_params = array(
                    ...
                    'messages' => array(
                        ...
                        $perc_size . '%', __('of the size of the "Single Product image", that is', 'yith-woocommerce-watermark'), $max_w, $max_h),
...
<div class="notice notice-info"
	style="padding-right: 38px; position: relative;">
	<p><?php echo $message; ?></p>
	<a class="notice-dismiss" href="<?php echo $url; ?>"
