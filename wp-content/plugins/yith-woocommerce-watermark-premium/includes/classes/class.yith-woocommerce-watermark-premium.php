...
if (! class_exists('YITH_WC_Watermark_Premium')) {
    ...
    class YITH_WC_Watermark_Premium extends YITH_WC_Watermark
    {
        ...
        public function __construct()
        {
            ...
            // add premium tabs
            add_filter('ywcwat_add_premium_tab', array(
                $this,
                'add_premium_tab'
            ));
            
            // add ajax action for load admin template
            add_action('wp_ajax_add_new_watermark_admin', array(
                $this,
                'add_new_watermark_admin'
            ));
            add_action('wp_ajax_nopriv_add_new_watermark_admin', array(
                $this,
                'add_new_watermark_admin'
            ));
            
            // add ajax action for add new watermark (in single product )
            add_action('wp_ajax_add_new_product_watermark_admin', array(
                $this,
                'add_new_product_watermark_admin'
            ));
            add_action('wp_ajax_nopriv_add_new_product_watermark_admin', array(
                $this,
                'add_new_product_watermark_admin'
            ));
            ...
            add_filter('ywcwat_max_item_task', array(
                $this,
                'remove_max_item_task'
            ));
        ...
        public function include_premium_style_script()
        {
            if (isset($_GET['page']) && 'yith_ywcwat_panel' == $_GET['page']) {
                ...
                wp_enqueue_script('ywcwat_premium_admin_script', YWCWAT_ASSETS_URL . 'js/ywcwat_premium_admin' . $this->_suffix . '.js', array(
                    'jquery',
                    'wp-color-picker'
                ), YWCWAT_VERSION, true);
                wp_enqueue_script('ywcwat_premium_enhanceselect', YWCWAT_ASSETS_URL . 'js/ywcwat_enhancedcselect' . $this->_suffix . '.js', array(
                    'jquery',
                    'select2'
                ), YWCWAT_VERSION, true);
            ...
            if (isset($post) && ('product' == $post->post_type || 'product_variation' == $post->post_type)) {
                ...
                wp_enqueue_script('ywcwat_premium_product_admin_script', YWCWAT_ASSETS_URL . 'js/ywcwat_admin_single_product' . $this->_suffix . '.js', array(
                    'jquery',
                    'wp-color-picker'
                ), YWCWAT_VERSION, true);
        ...
        public function add_new_watermark_admin()
        {
            if (isset($_REQUEST['ywcwat_addnewwat']) && isset($_REQUEST['ywcwat_unique_id'])) {
                ...
                wp_send_json(array(
                    'result' => yith_wcwat_get_template('single-watermark-template.php', $params, true)
                ));
            }
        }
        ...
        public function add_new_product_watermark_admin()
        {
            if (isset($_REQUEST['ywcwat_product_addnewwat'])) {
                ...
                wp_send_json(array(
                    'result' => yith_wcwat_get_template('single-product-watermark-template.php', $params, true)
                ));
            }
        }
            ...
            if (is_file($filepath)) {
                ...
                foreach ($watermarks as $key => $watermark) {
                    ...
                    if (! empty($watermark_category)) {
                        ...
                        $watermark_category = explode(',', $watermark['ywcwat_watermark_category']);
        ...
        public function save_product_watermark_meta($post_id, $post)
        {
            if (isset($_REQUEST['ywcwat_custom_watermark'])) {
                ...
                update_post_meta($post_id, '_ywcwat_product_watermark', $custom_watermark);
            } else {
                delete_post_meta($post_id, '_ywcwat_product_watermark');
            }
            
            if (isset($_REQUEST['_ywcwat_product_enabled_watermark'])) {
                
                update_post_meta($post_id, '_enable_watermark', 'yes');
            } else {
                update_post_meta($post_id, '_enable_watermark', 'no');
            }
        }
        ...
        public function generate_watermark_after_change_image($post_id, $attachment_id = -1)
        {
            ...
            if ($post && ($post->post_type == 'product' || $post->post_type == 'product_variation')) {
                ...
                $is_custom_enabled = get_post_meta($post->ID, '_enable_watermark', true) == 'yes';
                
                $custom_watermark = get_post_meta($post->ID, '_ywcwat_product_watermark', true);
                $watermark = null;
